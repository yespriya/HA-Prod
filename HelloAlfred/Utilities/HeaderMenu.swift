...
class HeaderMenuView: UIView {
    ...
    func getUserProfileImgApiCall() {
        profileViewModel.fetchUserDetails() { [weak self] success in
            if success {
                let profileURL = profileViewModel.profileDetailsRes?.profile_url ?? ""
                ...
                if(urlString != "NA") {
                    if let imageUrl = URL(string: urlString ?? "") {
                        ...
                        loadingIndicator.center = self?.profileImgView.center ?? .zero
                        self?.profileImgView.addSubview(loadingIndicator)
                        loadingIndicator.startAnimating()
                        
                        // Load the image asynchronously using SDWebImage
                        self?.profileImgView.sd_setImage(with: imageUrl) { (_, _, _, _) in
                    ...
                    if (profileViewModel.profileDetailsRes?.gender == "Male") {
